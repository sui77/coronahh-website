...
class Affenpocken extends AbstractController {

    protected $_template = 'affenpocken';
    ...
    protected function colorInzidenz($value) {
        ...
        $colr = '';
        if ($value > 0.5) { $colr = $colors[0]; }
        if ($value > 1) { $colr = $colors[1]; }
        if ($value > 1.5) { $colr = $colors[2]; }
        if ($value > 2) { $colr = $colors[3]; }
        if ($value > 2.5) { $colr = $colors[4]; }
        if ($value > 3) { $colr = $colors[6]; }
        if ($value > 3.5) { $colr = $colors[8]; }
        if ($value > 4) { $colr = $colors[9]; }
        if ($value > 4.5) { $colr = $colors[11] . ';color:#aaa'; }
        if ($value > 5) { $colr = $colors[12] . ';color:#aaa'; }
        if ($value > 5.5) { $colr = $colors[13] . ';color:#aaa'; }
        if ($value > 6) { $colr = $colors[14] . ';color:#aaa'; }
        if ($value > 6.5) { $colr = $colors[15] . ';color:#aaa'; }
        if ($value > 7) { $colr = $colors[16] . ';color:#aaa'; }
        if ($value > 7.5) { $colr = $colors[17] . ';color:#aaa'; }
        if ($value > 8) { $colr = $colors[18] . ';color:#aaa'; }
        if ($value > 8.5) { $colr = $colors[19] . ';color:#aaa'; }
    ...
    public function csv() {
        ...
        $sql = "SELECT * FROM alter_rki_ap ORDER BY year,week asc";
